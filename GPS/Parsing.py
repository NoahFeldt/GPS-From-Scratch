...
def sort_gps(messages1002: list[RTCM1002], messages1019: list[RTCM1019]) -> list[GPSSatellite]:
    ...
    longitudes_of_ascending_node: list[list[float]] = []
    ...
    for i in range(0, len(all_svs)):
        ...
        longitudes_of_ascending_node.append([])
    ...
    for i in range(0, len(messages1019)):
        ...
        if times_of_ephemeris[index].count(messages1019[i].time_of_week) < 1:
            ...
            longitudes_of_ascending_node[index].append(messages1019[i].longitude_of_ascending_node)
    ...
    # Set basis time
    # basis_time = get_basis_time(times_of_ephemeris)

    # for i in range(0, len(times_of_pseudoranges)):
    #     for j in range(0, len(times_of_pseudoranges[i])):
    #         times_of_pseudoranges[i][j] -= basis_time

    # for i in range(0, len(times_of_ephemeris)):
    #     for j in range(0, len(times_of_ephemeris[i])):
    #         times_of_ephemeris[i][j] -= basis_time

    # create list og GPSSatellite objects
    gps_list: list[GPSSatellite] = []

    for i in range(0, len(all_svs)):
        gps = GPSSatellite(pseudoranges[i], times_of_pseudoranges[i], eccentricities[i], inclinations[i], mean_anomalies[i], semi_major_axes[i], longitudes_of_ascending_node[i], arguments_of_periapsis[i], times_of_ephemeris[i])
